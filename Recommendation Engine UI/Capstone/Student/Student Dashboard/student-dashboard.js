...
function vaaLocalFillProgressBar(progress) {
	var width = 0;
...
function vaaFillProgressBar(degreeData) {
	var studentDegreeData = JSON.parse(sessionStorage.getItem(degreeData));
	var index = 0;
	var count = 0;
	for (; index < studentDegreeData.length; index++) {
		if (studentDegreeData[index].status == 'completed') {
			counter++;
		}
	}
	var progress = counter / studentDegreeData.length;
	var width = 0;
	var id = setInterval(frame, 30);
	function frame() {
		if (width >= progress) {
